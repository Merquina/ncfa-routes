...
self.addEventListener('activate', (event) => {
  event.waitUntil(
    caches.keys().then((keys) =>
      Promise.all(keys.filter((k) => k.startsWith('app-shell-') && k !== APP_SHELL_CACHE).map((k) => caches.delete(k)))
  ...
  event.respondWith(
    caches.match(request).then((cached) => {
      ...
      return fetch(request).then((res) => {
        const resClone = res.clone();
        caches.open(APP_SHELL_CACHE).then((cache) => cache.put(request, resClone)).catch(() => {});
        return res;
      }).catch(() => cached);
    })
  );
